...
Projet MDD (« Monde de Dev »), OpenClassrooms P5 option B. Rapport établi le 19 septembre 2026 sur le code de `main` au commit `d2cdd31` ; les renvois à `TESTS_REVIEW.md` visent son état après le correctif de décomptes du même jour (commit `d754d38`).
...
| **B. Contrôleur** (`@WebMvcTest` + `@Import({SecurityConfig, GlobalExceptionHandler})`, services et `JwtDecoder` en `@MockitoBean`) | `UserControllerIT` 30, `PostControllerIT` 14, `AuthControllerIT` 12, `TopicControllerIT` 2 — **58** | Routage et codes HTTP, validation `@Valid` et contenu de `fieldErrors`, mapping exception → statut par `GlobalExceptionHandler`, extraction de l'identité depuis `sub`, 401 sans token, 400 sur paramètre non numérique, absence de `password`/`passwordHash` dans les réponses. | Le service réel (mocké), le décodage réel d'un JWT (§7), le rendu réel de la page `/error`. |
...
Total : **126 tests** = 45 Surefire + 81 Failsafe, 0 échec, 0 erreur, 0 ignoré [mesuré].
...
Anomalies corrigées : aucune mutation n'a mis au jour un bug **réel** du code de production — les quinze cas sont des trous de filet, pas des défauts. Les corrections portent donc exclusivement sur `src/test/java`, ce que garantit le contrôle `git diff src/main/java` vide en fin de chantier. Les axes relevés par la revue technique (`REVUE_TECHNIQUE.md` §2) ne sont **pas** corrigés dans le périmètre du MVP, à l'exception de la Javadoc ; chacun y est associé à une recommandation chiffrée.
...
- **Validation d'entrée** : 15 tests de corps invalide (400) avec, pour six d'entre eux, la clé et le message exacts de `fieldErrors` ; 4 tests de paramètre d'URL invalide (trois id non numériques, un `sort` hors `asc|desc`) [mesuré].
...
| **Inscription sans mot de passe non testée.** `RegisterRequest.password` n'a pas de `@NotBlank` ; un corps sans ce champ passe la validation et atteint `passwordEncoder.encode(null)` → 500 hors `ErrorResponse`. Aucun des huit `register_*` de `AuthControllerIT` ne l'envoie. | `RegisterRequest.java:35-40`, `AuthService.java:46` [code] | Anomalie réelle relevée par la revue technique (axe p), hors de portée de la preuve par mutation qui ne renforce que des tests existants. | Correction et test : revue technique, recommandation p |
